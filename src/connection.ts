...
export class ConnectionManager implements IConnectionManager {
  ...
  public start(connectionConfig: IConnectionConfiguration) {
    ...
    var contextPath = this.extensionContext.asAbsolutePath(
      path.join('vendor', 'languageserver', 'puppet-languageserver')
    );
    ...
    if (
      this.connectionConfiguration.host == '127.0.0.1' ||
  ...
  private createLanguageServerProcess(serverExe: string, callback: Function) {
    ...
    if (localServer == null) {
      localServer = RubyHelper.getRubyEnvFromPuppetAgent(serverExe, this.connectionConfiguration, this.logger);
    }
